            ...
            int* mem_accesses_per_elem)
{
  ...
  *mem_accesses_per_elem = 2;
  ...
  uint64_t i, j;
  for (j = 0; j < ntrials; ++j) {
  for (i = 0; i < nsize; i=i+blocksize*sizeof(*A)) {
     for (int k = 0; )
	 memcpy(&A[i],&B[0],blocksize*sizeof(*A));
	 msync(&A[i], blocksize*sizeof(double), MS_SYNC);
  }
    alpha = alpha * (1 - 1e-8);
  }
}
...
//#pragma omp parallel private(id) 
		{
				...
				#pragma omp barrier
								...
								if ((id == 0) && (rank == 0)) {
										...
										//       total_bytes,
										//       total_flops);
                                        printf("BW: %15.3lf Total data: %15.3lfG \n",total_bytes*1.0/seconds/1.024/1.024/1024/2, total_bytes*1.0/1024/1024/1024);
								} // print
					it++; 
				} // working set - nsize
		} // parallel region
		FILE * fp = fopen("bak","w+");
		for (int i = 0; i < PSIZE/ sizeof(double); ++i)
			fprintf(fp, "%lf\n", buf[i]);
		free(buf);
		//munmap(buf, TSIZE);
		//close(fd);
